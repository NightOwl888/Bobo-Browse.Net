...
{
    public class BrowseServiceImpl : BrowseService
    {
        private static readonly log4net.ILog logger = LogManager.GetLogger(typeof(BrowseServiceImpl));
        private readonly FileInfo idxDir;
        private readonly BoboIndexReader reader;

        public BrowseServiceImpl(FileInfo idxDir)
        {
            this.idxDir = idxDir;
            try
            {
        ...
        {
            Directory idxDir = FSDirectory.GetDirectory(this.idxDir);
